...
from edarop.edarop import EdaropCAllocator, EdaropRAllocator
...
class TestEdaropCBasic:
    ...
    def test_basic_infeasible(self):
...
class TestEdaropC2CloudRegions2EdgeRegions2Apps:
    """Tests for cost optimization with only two cloud regions and two edge
    regions and two apps. It is based on prices and performances from Amazon and
    Equinix."""
...
class TestEdaropR2CloudRegions2EdgeRegions2Apps:
    """Tests for response time optimization with only two cloud regions and two
    edge regions and two apps. It is based on prices and performances from
    Amazon and Equinix."""
    ...
    def __set_up(self):
        ...
        self.workloads = {
            ...
            (app_a0, region_madrid): Workload(
                values=[6000, 4000, 4000, 0, 15000, 300],
                time_unit=TimeUnit("h"),
            ),
            #
            (app_a1, region_dublin): Workload(
    ...
    def test_r_2CloudRegions2EdgeRegions2Apps_feasible(self):
        ...
        assert avg_resp_time == pytest.approx(0.145555273442813)
        assert sol.status == Status.OPTIMAL

    def test_r_2CloudRegions2EdgeRegions2Apps_infeasible(self):
        """Test a system that is infeasible."""
        self.__set_up()
